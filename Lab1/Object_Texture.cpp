...
Object_Texture::Object_Texture(const std::string& fileName)
{
	int imageWidth, imageHeight, imageComponents; //imageWidth, imageHeight, and no of components of image
	unsigned char* imageData = stbi_load((fileName).c_str(), &imageWidth, &imageHeight, &imageComponents, 4); //load the image and store the data
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // wrap texture outside imageWidth
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // wrap texture outside imageHeight
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageWidth, imageHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData); //Target, Mipmapping Level, Pixel Format, Width, Height, Border Size, Input Format, Data Type of Texture, Image Data
...
void Object_Texture::bindShader(unsigned int unit)
{
